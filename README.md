...
- **SEO/AIO live (2026-07-03, PR #4):** homepage-only JSON-LD `@graph` (SoftwareApplication + FAQPage + WebSite) injected in `Base.astro` via `set:html` (gated on the `isHome` prop / root path — do NOT let it render on `/privacy` or `/terms`); keyword title + long meta description in `Base.astro`. All FAQ/metadata copy is **safety-locked** (Sage-approved) — never reword it without a fresh Sage pass. Source spec lives in the business repo: `marketing/content/drafts/2026-07-03-seo-structured-data.md`.
- **FAQ accordion + 7 visible questions (2026-07-03, PR #5):** the `§E2` FAQ in `index.astro` (between §E and §F) is now a **folded `<details>/<summary>` accordion** with **7 visible questions** (was 5 always-open). Answers stay in the DOM when folded (CSS `grid-template-rows: 0fr`, never `display:none`) — SEO/AIO requirement. The 7 visible Q&As now match the 7 JSON-LD `FAQPage` Q&As 1:1 (Google "markup matches visible content"). CSS-drawn plus→minus marker, terracotta hover/open, zero-JS grid-rows open animation + a small close shim (with a `prefers-reduced-motion` early-return so reduced-motion closes instantly, never wedges). Spec: `design/website/2026-07-03-faq-accordion-spec.md`; copy/order: `marketing/plans/2026-07-03-seo-aio-plan.md` §7. Copy remains Sage-locked. Minor open note: JSON-LD `Question.name` wording differs slightly from the visible `<h3>` for Q1-4/Q7 (pre-existing, non-blocking; strict parity would be a separate Beacon copy ticket).
